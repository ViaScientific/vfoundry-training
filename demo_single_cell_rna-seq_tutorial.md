...
Please go to https://viafoundry.com and log in to your account. If you have any issues with logging in, please let us know (support@viascientific.com). We will set-up an account for you.
...
and in the `3. Collection Type` dropdown, (14) select 'Paired List'. The `Amazon Keys for S3` box and list of samples should appear (blue boxes). The `Amazon Keys for S3` should automatically set itself to "Via Scientific AWS Key". Leave `2. File Type` as "FASTQ".
...
Pick the genome by (27) selecting "human_hg38_genecode_v32_cellranger_v6" in the `genome_build` dropdown. To customize the genome, (28) set `run_mkref` to "Yes" and (29) set `add_sequences_to_reference` to "Yes". To supply the new sequences (30) click the wrench in the `add_sequences_to_reference` section.
...
In the `Enter File` window (32) paste the following path into `File Location`:
